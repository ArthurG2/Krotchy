...
def knbk(message):
    """
    Ф Функция которая симулирует игру в камень ножницы бумагу с ботом. Эта функция симулирует развитие игры,если игрок выиграл камень путём случайного выбора того, что у бота(К, Н или Б)
    :param message: сообщение
    :return: Строка, которая говорит игроку его результат в игре в зависимости от того, что выпало боту(КМН выпадает ему случайно)
    """
...
def knbn(message):
    """
     Функция которая симулирует игру в камень ножницы бумагу с ботом. Эта функция симулирует развитие игры,если игрок выиграл ножницы путём случайного выбора того, что у бота(К, Н или Б)
    :param message: сообщение
    :return: Строка, которая говорит игроку его результат в игре в зависимости от того, что выпало боту(КМН выпадает ему случайно)
    """
    listkmn = ['Камень', 'Ножницы', 'Бумага']
...
def knbb(message):
    """
    Функция которая симулирует игру в камень ножницы бумагу с ботом. Эта функция симулирует развитие игры,если игрок выиграл бумагу путём случайного выбора того, что у бота(К, Н или Б)
    :param message: сообщение
    :return: Строка, которая говорит игроку его результат в игре в зависимости от того, что выпало боту(КМН выпадает ему случайно)
    """
    listkmn = ['Камень', 'Ножницы', 'Бумага']
...
def Krotchy(s):
    """
    Эта функция считывает данные с википедии в slovo, ограничивает его 700 символами, разделяет предложения по точкам, ограничивает slovo на последнем предложении до 700 символа.
    :return: Статья с википедии в случае если всё прошло успешно, определенная фраза - если статья в википедии не была найдена
    """
    try:
...
def start(message):
    """
    Функция, которая отвечает за главное меню бота. Здесь расположена панель навигации по функциям бота, а так же бот представляется и говорит, что умеет.
    :return: Ознакомительная фраза и кнопки
    """
...
def handle_text(message):
    """
    Функция которая отвечает за всё, что умеет бот помимо нахождения статей в википедии. Орёл и решка, факты, поддержка, переход во вкладки игры, игра в правду или действие и КМН.
    :param message: Сообщение
    :return: фразы и навигация по боту
    """
    list = ["Орёл", "Решка"]
